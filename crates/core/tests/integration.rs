...
fn test_full_initialization() {
    ...
    assert!(result.is_ok(), "WindowManager initialization should succeed");
    
    // After initialization, we should have at least one monitor
    assert!(!wm.get_monitors().is_empty(), "Should have at least one monitor after initialization");
}
...
fn test_monitor_enumeration() {
    ...
    for monitor in monitors {
        ...
        assert!(monitor.work_area.width > 0, "Monitor work area width should be positive");
        assert!(monitor.work_area.height > 0, "Monitor work area height should be positive");
        assert!(monitor.dpi_scale > 0.0, "Monitor DPI scale should be positive");
    }
}
...
fn test_workspace_management() {
    ...
    wm.switch_workspace(1).expect("Failed to switch to same workspace");
    assert_eq!(wm.get_active_workspace(), 1);
}
...
fn test_workspace_tree_access() {
    ...
    for workspace_id in 1..=10 {
        assert!(wm.get_workspace_tree(workspace_id).is_some(), 
            "Workspace {} should be accessible", workspace_id);
        assert!(wm.get_workspace_tree_mut(workspace_id).is_some(),
            "Workspace {} should be mutably accessible", workspace_id);
    }
    
    // Non-existent workspace should return None
    assert!(wm.get_workspace_tree(999).is_none(),
        "Non-existent workspace should return None");
}
...
fn test_window_management_lifecycle() {
    ...
    println!("WindowManager initialized with {} monitors", wm.get_monitors().len());
    ...
    if managed_count > 0 {
        ...
        wm.switch_workspace(1).expect("Failed to switch back to workspace 1");
        assert_eq!(wm.get_active_workspace(), 1);
    }
...
fn test_monitor_refresh() {
    ...
    wm.refresh_monitors().expect("Failed to refresh monitors again");
    let second_count = wm.get_monitors().len();
    
    // Monitor count should be consistent (unless monitors were physically added/removed)
    assert_eq!(initial_count, second_count, "Monitor count should be consistent");
}
...
fn test_workspace_tree_management() {
    ...
    for workspace_id in 1..=10 {
        ...
        assert!(tree.is_some(), "Workspace {} should have a tree", workspace_id);
    }
}
